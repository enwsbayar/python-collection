...
def nb_months(start_price_old, start_price_new, saving_per_month, percent_loss_by_month):
  ...
  loss_percent = percent_loss_by_month
  ...
  while True:
    ...
    old_price *= (1 - loss_percent / 100)
    new_price *= (1 - loss_percent / 100)
    
    savings += saving_per_month
    
    available = old_price + savings - new_price
    if available >= 0:
      return [months, int(round(available))]