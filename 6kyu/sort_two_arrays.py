...
def sort_two_arrays(arr1, arr2):
    sorted_indices_2 = sorted(range(len(arr2)), key=lambda i: (arr2[i], i))
    new_arr1 = [arr1[i] for i in sorted_indices_2]

    sorted_indices_1 = sorted(range(len(arr1)), key=lambda i: (arr1[i], i))
    new_arr2 = [arr2[i] for i in sorted_indices_1]

    return [new_arr1, new_arr2]

print(sort_two_arrays([5,6,9,2,6,5],[3,6,7,4,8,1]))
