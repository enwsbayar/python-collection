...
def choose_best_sum(t, k, ls):
    # If we need more towns than available, it's impossible
    if k > len(ls):
        return None
    
    best_sum = None
    
    # Generate all combinations of k distances
    for combo in combinations(ls, k):
        current_sum = sum(combo)
        # Check if this combination is within the limit
        if current_sum <= t:
            # Update best_sum if this is better
